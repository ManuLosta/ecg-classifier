...
class SeptalAnteriorExpert(TerritoryExpert):
    ...
    def __init__(self):
        super(SeptalAnteriorExpert, self).__init__(
            num_leads=4, expert_name="SeptalAnterior"
        )
...
class ECGTerritoryMoE(nn.Module):
    ...
    def forward(self, inputs: List[torch.Tensor]):
        ...
        septal_anterior_leads = [
            lead_outputs[i] for i in self.lead_territories["septal_anterior"]
        ]
...
class ECGTerritoryMoESingleInput(nn.Module):
    ...
    def forward(self, x):
        ...
        if x.shape[1] == 12:  # (batch, 12, height, width)
            ...
            raise ValueError(
                f"Input shape {x.shape} not supported. Expected last dim or second dim to be 12."
            )
...
def create_ecg_territory_moe(
    input_shape: Tuple[int, int, int], num_classes: int, single_input: bool = False
) -> nn.Module:
    """
...
def save_territory_moe_model(
    model: nn.Module, filepath: str, model_state_dict_only: bool = True
):
    """
...
def save_territory_moe_model_to_trained_dir(
    model: nn.Module, model_name: str, models_dir: str = "models/trained"
):
    """
...
def save_multibranch_model_to_trained_dir(
    model: nn.Module, model_name: str, models_dir: str = "models/trained"
):
    """
...
def calculate_class_weights(
    y_train, num_classes: int, normal_class_idx: int = 3, mi_class_idx: int = 2
) -> torch.Tensor:
    """
